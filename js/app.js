...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Configuración de los nodos neuronales
  const numNodes = 75;
  const nodes = [];
  const connectionDistance = 120; // Distancia máxima para conectar nodos

  // Inicializar neuronas con posiciones y velocidades aleatorias
  ...
  function animate() {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      // Dibujar el punto de la neurona
      ctx.beginPath();
      ctx.arc(node.x, node.y, node.radius, 0, Math.PI * 2);
      ctx.fillStyle = '#38bdf8'; // Azul neón cyber
      ctx.shadowBlur = 8;
      ...
      for (let j = i + 1; j < nodes.length; j++) {
        ...
        // Si están lo suficientemente cerca, dibuja la línea de conexión
        if (dist < connectionDistance) {
          ctx.beginPath();
          ctx.moveTo(node.x, node.y);
          ctx.lineTo(other.x, other.y);
          
          // La línea se desvanece (opacidad) a mayor distancia
          const alpha = 1 - dist / connectionDistance;
          ctx.strokeStyle = `rgba(56, 189, 248, ${alpha})`;
          ctx.lineWidth = 0.6;
          
          ctx.save();
          ctx.shadowBlur = 0; // Desactivar sombra pesada en líneas para mejorar rendimiento
          ctx.stroke();
          ctx.restore();
        }
      }
